...
import {  CacheCommandHandler } from "../commands/cacheCommandHandler";
...
export const conversationBot = new ConversationBot({
  ...
  command: {
    enabled: true,
    commands: [new HelloWorldCommandHandler(), new CacheCommandHandler()],
  },
  cardAction: {
    enabled: true,
    actions: [new DoStuffActionHandler(), new SubmitActionHandler()],
  },
});
